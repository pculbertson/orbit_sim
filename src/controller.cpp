...
Controller::Controller(int input, ros::NodeHandle n){
	...
	this->Kx = 1e-6*MatrixXd::Identity(3,3);
	this->Kr = 1e-6*MatrixXd::Identity(3,3);
	this->J = 1e-6*MatrixXd::Ones(3,6);
	this->gX = 1e1*MatrixXd::Identity(3,3);
	this->gR = 1e0*MatrixXd::Identity(3,3);
	this->gJ = 1e-1*MatrixXd::Identity(3,3);
...
void Controller::refCallback(const orbit_sim::refSig::ConstPtr& _msg){
	if (refTime == 0){
		...
		this->xM = .85*Vector3d(_msg->xm.x,_msg->xm.y,_msg->xm.z)+.15*this->xM;
		this->r = .85*Vector3d(_msg->r.x,_msg->r.y,_msg->r.z)+.15*this->r;
		this->refTime = ros::Time::now().toSec();
	}
...
void Controller::stateCallback(const gazebo_msgs::LinkStates::ConstPtr& _msg){
	if (this->stateTime==0){
		...
		this->J = this->J + dt*(-this->gJ*this->Bm.transpose()*this->P*e*(this->fOmega(this->x)).transpose()-5e-10*this->J);
